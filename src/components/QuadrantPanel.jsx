...
// Categories where the left panel genuinely shows location-specific results
const LOCATION_CATEGORIES = ['restaurant', 'local', 'health', 'hotel'];

export default function QuadrantPanel({ type, city, country, results = [], query, category }) {
  const isLocal = type === 'local';
  const accentColor = isLocal ? '#0EB87B' : '#5254E8';
  const tintColor = isLocal ? '#E6FBF3' : '#EEEEFF';
  const isLocationBased = LOCATION_CATEGORIES.includes(category);
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {isLocal ? (
            // Pin icon for location-based, flag icon for regional
            isLocationBased ? (
          ...
          <span style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 15, color: '#0A0A12' }}>
            {isLocal
              ? isLocationBased
                ? `3 best near ${city || 'you'}`
                : `3 best in ${country || 'UK'}`
              : '3 best worldwide'
            }
          </span>
        </div>
